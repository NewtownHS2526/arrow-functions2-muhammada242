...
// Convert to arrow function (convert both outer and inner functions):
const processData = (data, transform) => data.reduce((acc, item) => { acc.push(transform(item)); return acc; }, []);
...
// Convert to arrow function (convert both outer and inner functions):
const groupBy = (array, keyFn) => array.reduce((groups, item) => { const key = keyFn(item); if (!groups[key]) { groups[key] = []; } groups[key].push(item); return groups; }, {});
